...
describe Digibot::Exchange::Binance do
  include_context 'mocked binance'

  before(:all) { Digibot::Log.define }

  let(:binance) do
    Digibot::Exchange::Binance.new(
      {
  ...
  context 'ojbect initialization' do
    it 'is a sublass of Digibot::Exchange::Base' do
      expect(Digibot::Exchange::Binance.superclass).to eq(Digibot::Exchange::Base)
    end

    it 'has an orderbook' do
      expect(binance.orderbook).to be_an_instance_of(Digibot::Orderbook)
    end
  end

  context 'getting snapshot' do
    let(:snapshot_buy_order_1) { Digibot::Order.new('ETHUSDT', 135.84000000, 6.62227000, :buy) }
    let(:snapshot_buy_order_2) { Digibot::Order.new('ETHUSDT', 135.85000000, 0.57176000, :buy) }
    let(:snapshot_buy_order_3) { Digibot::Order.new('ETHUSDT', 135.87000000, 36.43875000, :buy) }

    let(:snapshot_sell_order_1) { Digibot::Order.new('ETHUSDT', 135.91000000, 0.00070000, :sell) }
    let(:snapshot_sell_order_2) { Digibot::Order.new('ETHUSDT', 135.93000000, 8.00000000, :sell) }
    let(:snapshot_sell_order_3) { Digibot::Order.new('ETHUSDT', 135.95000000, 1.11699000, :sell) }
  ...
  context 'binance message processing' do
    ...
    let(:example_messsage_buy_order_1) { Digibot::Order.new('ETHUSDT', 136.07000000, 29.38270000, :buy) }
    let(:example_messsage_buy_order_2) { Digibot::Order.new('ETHUSDT', 136.43000000, 0.66174000, :buy) }

    let(:example_messsage_sell_order_1) { Digibot::Order.new('ETHUSDT', 136.44000000, 5.15285000, :sell) }
    let(:example_messsage_sell_order_2) { Digibot::Order.new('ETHUSDT', 136.45000000, 165.29973000, :sell) }
    let(:example_messsage_sell_order_3) { Digibot::Order.new('ETHUSDT', 136.50000000, 0.16122000, :sell) }
    let(:example_messsage_sell_order_4) { Digibot::Order.new('ETHUSDT', 136.51000000, 0.93508000, :sell) }
    let(:example_messsage_sell_order_5) { Digibot::Order.new('ETHUSDT', 136.52000000, 25.20000000, :sell) }
    ...
    context 'price level removing' do
      let(:order_to_remove) { Digibot::Order.new('ethusdt', 136.07000000, 29.38270000, :buy) }
  ...
  context 'order_create' do
    let(:order) { Digibot::Order.new('ETHUSDT', 250, 1, :buy) }
