describe Digibot::OpenOrders do
  let(:market)          { 'ethusd' }
  let(:open_orders)     { Digibot::OpenOrders.new(market) }
  let(:delete_order)    { { id: 1, order: Digibot::Order.new(market, 100, 30, :buy) } }
  let(:skip_order)      { { id: 2, order: Digibot::Order.new(market, 200, 20, :sell) } }
  let(:create_order)    { { id: 3, order: Digibot::Order.new(market, 250, 20, :sell) } }
  let(:update_order)    { { id: 4, order: Digibot::Order.new(market, 500, 10, :buy) } }
  let(:update_order_ob) { { id: 5, order: Digibot::Order.new(market, 500, 5, :buy) } }
