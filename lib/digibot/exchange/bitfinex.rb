...
module Digibot::Exchange
  class Bitfinex < Base
    ...
    def build_order(data)
      ...
      Digibot::Order.new(@market, price, amount, side)
    end
    ...
    def process_event_message(msg)
      ...
      when 'auth'
      when 'subscribed'
        Digibot::Log.info "Event: #{msg['event']}"
      when 'unsubscribed'
      when 'info'
      when 'conf'
      when 'error'
        Digibot::Log.info "Event: #{msg['event']} ignored"
      end
    end
    ...
    def on_open(e)
      ...
      Digibot::Log.info 'Open event' + sub.to_s
      EM.next_tick {
    ...
    def on_close(e)
      Digibot::Log.info "Closing code: #{e.code} Reason: #{e.reason}"
    end
